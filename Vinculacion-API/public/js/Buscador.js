document.addEventListener("DOMContentLoaded", function () {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            alert(
              "Error al actualizar la empresa: " + response.data.message
            );
          }
        })
        .catch((error) => {
    ...
    // --- Lógica para agregar una nueva empresa a través de un Modal ---
    ...
    // Funciones para mostrar y ocultar el modal "Agregar Nueva Empresa"
    ...
    btnAddNewCompany.addEventListener("click", showAddModal);
    cancelAddCompany.addEventListener("click", hideAddModal);
  
    // Manejador para el envío del formulario de agregar nueva empresa.
    addCompanyForm.addEventListener("submit", function (e) {
      e.preventDefault();
      const formData = new FormData(addCompanyForm);
      const data = Object.fromEntries(formData.entries());
      axios
        .post("/add-empresa", data)
        .then((response) => {
          if (response.data.success) {
            alert("Empresa agregada correctamente");
            hideAddModal();
            addCompanyForm.reset();
            // Refrescar la lista para incluir la nueva empresa.
            cargarEmpresas(searchInput.value.trim());
          } else {
            alert("Error al agregar la empresa: " + response.data.message);
          }
        })
        .catch((error) => {
    ...
    // Seleccionar todos los inputs del modal de agregar empresa
    const inputs = document.querySelectorAll("#addCompanyModal input[type='text']");

    // Aplicar la transformación a mayúsculas cuando el usuario escribe
